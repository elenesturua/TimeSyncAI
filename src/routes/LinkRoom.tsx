...
export default function LinkRoom() {
  ...
  const loadParticipants = async () => {
    ...
    try {
      const data = await linkApi.getParticipants(id);
  ...
  const getSuggestions = async () => {
    ...
    try {
      const response = await linkApi.getSuggestions(id, 45, allowAbsences);
  ...
  const handleBook = async (booking: any) => {
    ...
    try {
      await withBearer(getAccessToken);
      const response = await linkApi.book(id, booking);
      navigate(`/l/${id}/success`, { state: { booking: response } });
    } catch (error) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Link Room</h1>
        <p className="text-gray-600">Connect your calendar and find the best time for your meeting.</p>
      </div>
          ...
          <div className="card">
            ...
            {isLoadingParticipants ? (
              ...
              <EmptyState
                ...
                description="Share the link to invite participants to connect their calendars."
              />
            ) : (
